...
export async function generateMetadata({
  ...
  const product = await getSingleProduct(identifier);
  ...
  return {
    title: product.name,
    description:
      stripHtml(product.shortDescription || product.description)?.slice(
        0,
        150,
      ) || "Buy premium Islamic clothing from Al Idaad",
...
const ProductDetailsPage = async ({
  ...
  const { id: identifier } = await params;
  const singleProductData = await getSingleProduct(identifier);
