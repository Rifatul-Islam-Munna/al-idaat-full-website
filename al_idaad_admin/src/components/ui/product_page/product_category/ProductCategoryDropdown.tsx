...
const ProductCategoryDropdown = () => {
    ...
    const { productCategories, productCategoriesLoading, changeProductCategoryKey } = useAuth();
    ...
    const handleDeleteClick = async (cat: Category) => {
        Swal.fire({
            title: "Are you sure?",
            text: `Delete product category "${cat.name}"? This cannot be undone.`,
            icon: "warning",
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    Swal.fire("Error!", error.response?.data?.message || "Failed to delete product category.", "error");
                } finally {
                    setDeletingId(null);
                }
            }
    ...
    if (productCategoriesLoading) return <div>Loading categories...</div>;
    if (!productCategories.length) return <div>No categories found</div>;
    ...
    return (
        <div>
            ...
            <AnimatePresence mode="wait">
                {selectedCategory && <UpdateProductCategoryModal key="update-modal" category={selectedCategory} onClose={closeModal} />}
            </AnimatePresence>
        </div>
    );
};
