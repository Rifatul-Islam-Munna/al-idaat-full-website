...
const UpdateBlogCategoryModal: React.FC<UpdateBlogCategoryModalProps> = ({ category, onClose }) => {
    const { changeBlogCategoryKey } = useAuth();
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await api.put(`/blog-categories/${formData._id}`, formData);

            if (res.data.success) {
                Swal.fire("Success", "Blog category updated!", "success");
                changeBlogCategoryKey(); // refetch categories
                onClose();
            }
        } catch (error: unknown) {
            console.log(error);
